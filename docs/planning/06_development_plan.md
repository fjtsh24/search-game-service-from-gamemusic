...
- [x] `import_steam_soundtracks.py` 修正済み（2026-07-20）
...
- [ ] `python3 scripts/import_steam_soundtracks.py --limit 200` を実行（既存200件を更新 + 高評価サントラを追加）
...
- [ ] GitHub Actions `daily-import.yml` の youtube ジョブで毎日自動実行（毎日 JST 10:00）
- [ ] 初回手動実行: `python3 scripts/import_youtube_video_ids.py --limit 200`
...
- [ ] 初回実行: `python3 scripts/import_game_tags.py --limit 200`
...
### 4-2. フロントエンド
- [x] ログイン後ホームページ（`FeedSection.tsx` でパーソナライズドフィード）
- [x] 未ログイン時のホームページ（タグクラウド + 人気ゲーム一覧）
...
- [x] Path B が本番環境で完走できる
- [x] Steam 認証が本番ドメインで動く
- [x] アカウント削除が動く
