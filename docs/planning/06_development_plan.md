...
> - **Last.fm `album.getTopTags`**: タグ付けに使用するスクリプトを実装済みだが、ゲームサントラのカバレッジが根本的に薄く89%のゲームでタグ取得失敗。Steam OSTページの説明文を入力ソースとしたタグ付けへの切り替えを検討中。
> - **Steam Music アプリ（OSTページ）**: `import_steam_ost_data.py` で実装済み。Steam に登録されたサントラアプリ（`type=music`）のページをスクレイピングしてトラックリスト・作曲家クレジットを取得。約40%のゲームでOSTアプリを発見。OSTページの description テキストをLLMへ渡したタグ付けを次フェーズで実装予定。
> - **VGMdb**: 非公式APIのため採用見送り。カラム削除済み（migration 012）。
...
### 処理内訳（合計 ~155件/日、拡充:新規 = 30:1）

| 優先度 | 処理 | 件数 | ソース | sleep |
|---|---|---|---|---|
| 1 | タグ未付与ゲームへのタグ付与 | 40件 | Last.fm（暫定） | 0.3s |
| 2 | 既存ゲームの欠損説明文補完（ja/zh） | 30件 | Steam appdetails | 1.2s |
| 3 | YouTube VideoID 欠損トラックの補完 | 20件 | YouTube Search | 0.1s |
| 4 | Steam 新規ゲーム追加 | 5件 | Steam Search | 1.2s |
| 5 | Steam OST appID 発見 | 50件 | Steam Search + appdetails | 1.0s |
| 6 | Steam OST ページ取得（トラック・作曲家） | 10件 | Steam ストアページ（HTML） | 120s |
...
| スクリプト | 対象の選び方 | 処理済み・失敗済みのスキップ |
|---|---|---|
| `import_game_tags.py` | `tags_locked=FALSE` かつ `game_tags` 未登録のみ。全件 locked or 登録済みならスキップ | locked フラグ |
| `import_steam_soundtracks.py --backfill` | `description/ja/zh` のいずれかが null のゲームを `updated_at ASC`（古い順）で取得。新規データが取得できなかった場合も `updated_at` を更新してキュー末尾に回す | `updated_at` 更新で自然にローテーション |
| `import_steam_soundtracks.py`（新規追加） | 実行開始時に既登録 `steam_app_id` を全件取得。スキャン中に既登録ゲームをスキップし Steam API 呼び出しを節約 | 事前取得済み ID セットで除外 |
| `import_youtube_video_ids.py` | Phase1: `youtube_video_id=null` かつ `youtube_locked=FALSE` のトラック。Phase2: tracks 未作成かつ `youtube_locked=FALSE` のゲーム | `youtube_locked` フラグ |
| `import_lastfm_similarities.py` | 毎回全作曲家を処理。`upsert` なので冪等 | （手動専用 / 件数少なく許容範囲） |
...
## データパイプライン品質改善（2026-07-22）

### バッチ優先度バグ修正
- [x] **`import_game_tags.py` 優先度修正**
  - 旧: `tags_locked=FALSE` のゲームを最大 `limit*2` 件取得して未タグでフィルタするだけで、0件になってもそのまま終了
  - 新: unlocked かつ未タグのゲームのみを対象。全件 locked または登録済みなら処理スキップ（同じゲームが延々と再選択される問題を解消）
- [x] **`import_steam_soundtracks.py --backfill` 優先度修正**
  - 旧: ORDER BY なし → Steam が翻訳を提供しないゲームが毎日同じ順番で選ばれ続ける
  - 新: `ORDER BY updated_at ASC`（最後に試行した日時が古い順）。新規データなしの場合も `updated_at` を更新してキュー末尾に回す
- [x] **`import_steam_soundtracks.py`（新規追加）重複登録修正**
  - 旧: 既登録ゲームに対して毎日 Steam API（review_score + appdetails×3）を呼び出し、UPDATE するだけで件数に計上していた
  - 新: 起動時に既登録 `steam_app_id` を全件取得し、スキャン・登録フェーズ両方で既登録をスキップ

### DB 品質改善
- [x] **Entropy: Zero 2 削除**（Half-Life 2 のモドであり自作ゲームでない）
- [x] **Grimm's Hollow 動画クリア**（`youtube_flagged=true` の報告済み動画を管理者が確認し削除）
  - `youtube_video_id = NULL`、`youtube_flagged = FALSE` に更新
- [x] **migration 008: UNIQUE 制約追加**
  - `tracks.(game_id, track_number)` — 同ゲーム内のトラック番号重複を DB レベルで防止

---

## ベータ後アップデート（2026-07-23）

### フロントエンド
- [x] **Vercel Analytics 組み込み**（PR #28）
  - `@vercel/analytics` パッケージ追加
  - `web/app/layout.tsx` に `<Analytics />` 配置

### インフラ・DB
- [x] **migration 011: is_discoverable DBトリガー追加**（PR #31）
  - `game_tags` の INSERT/DELETE 時、`tracks.youtube_video_id` 変更時に `is_discoverable` を自動再計算
  - タグ・動画削除時も自動で FALSE に戻るよう整合性を担保

### バッチスクリプト品質改善（PR #32）
- [x] **全スクリプトに User-Agent 統一**
  - `import_game_tags.py` / `import_youtube_video_ids.py` / `import_lastfm_similarities.py` を `requests.Session` + `User-Agent: GameMusicDiscovery/0.1.0` に統一（`import_steam_soundtracks.py` と同パターン）
- [x] **`import_lastfm_similarities.py` HTTPエラー処理追加**
  - `resp.ok` チェックと `"error"` キー確認を追加。HTTPエラー時の JSONDecodeError でスクリプト全体が落ちる問題を修正

### DB手動対応
- [x] **星空列車与白の旅行 YouTube動画修正**
  - 誤動画（`rlvR9HjaoN4`、OPテーマ1曲のみ）を公式チャンネルのOP動画（`MEJC0FsCido`）に差し替え
  - `youtube_flagged` を FALSE にリセット
...
**✅ 実装済み（2026-07）: `import_steam_ost_data.py`**

Steam に登録されたサントラアプリ（`type=music`）のストアページをスクレイピング。
- discover フェーズ: Steam Search + appdetails API で `steam_ost_appid` を発見・保存
- scrape フェーズ: OSTページの HTML からトラック名・尺・作曲家クレジットを取得
- 約40%のゲームで Steam OST アプリを発見。`tracks.duration_seconds` と `composers` / `track_composers` を日次バッチで蓄積中。
...
├── api/              # Python + FastAPI
├── supabase/
│   └── schema.sql    # DB スキーマ定義（git 履歴がマイグレーション履歴を兼ねる）
...
- [x] **migration 012: 不採用カラム削除**
  - `games.vgmdb_album_id` / `games.musicbrainz_release_id` を DROP（両サービスとも採用見送りが確定）
- [x] **migration 013: Steam OST カラム追加**
  - `games.steam_ost_appid` — サントラの Steam Music アプリID
  - `games.steam_ost_scraped_at` — OST ページスクレイピング済み日時

---

## アップデート（2026-07-24）

### YouTube 動画アーキテクチャ刷新（PR #40）

**変更前**: OST全体の動画IDを `tracks.youtube_video_id` で管理（最初の1件を疑似的に使用）  
**変更後**: OST全体（`games.youtube_video_id`）とトラック別（`tracks.youtube_video_id`、将来実装）に明確に分離

- `games.youtube_video_id` / `games.youtube_flagged` カラム追加（migration 014）
- `import_youtube_video_ids.py` 刷新 — `games.youtube_video_id` のみを更新する単一フェーズに統合、`--phase` 引数廃止
- `POST /games/{id}/flag-video` が `games.youtube_flagged` を更新するよう修正
- `daily-import.yml` Step 7 の参照先を `tracks.youtube_flagged` → `games.youtube_flagged` に修正（バグ修正）
- is_discoverable トリガーを `games.youtube_video_id` の変更にも対応（migration 015）

### トラックリスト UI 追加（PR #40）

`YouTubePlayer.tsx` を刷新:
- YouTubeプレーヤーとトラックリストを横並びレイアウト（`lg:flex-row`）
- トラックリストは固定高さ + スクロール（「音楽的に似たゲーム」が下に押し出される問題を解消）
- `tracks.youtube_video_id` がある曲のみ `▶` バッジを表示（将来のトラック別動画向け）

### スキーマ一元管理（PR #40）

`supabase/migrations/`（15ファイル）を廃止。`supabase/schema.sql` 1ファイルで管理。  
git 履歴がマイグレーション履歴を兼ねる。psql で実 DB と照合済み（差分ゼロ）。

**スキーマ変更手順（今後）**:
1. Supabase ダッシュボードまたは psql で `ALTER TABLE` 等を実行
2. `supabase/schema.sql` を手動更新
3. git commit

### daily-import バグ修正・改善（PR #60）

| 対象 | 種別 | 内容 |
|---|---|---|
| `daily-import.yml` Step 3 | バグ修正 | `--phase 1` を削除（スクリプトが未定義引数でエラー） |
| `daily-import.yml` Step 7 | バグ修正 | `tracks.youtube_flagged` → `games.youtube_flagged` |
| `import_game_tags.py` | バグ修正 | キャッシュクリアを GET URL → POST body 方式に変更（URL too long 防止） |
| `import_game_tags.py` | 改善 | `is_discoverable=True` 手動 UPDATE を削除（トリガーが自動処理） |
| `import_lastfm_similarities.py` | 改善 | `--limit N` 追加・未登録作曲家（OST 新規追加分）を優先処理 |
| `daily-import.yml` Step 8 | 改善 | `import_lastfm_similarities.py --limit 30` を日次パイプラインに追加 |

### 日次パイプライン（2026-07-24 時点）

| Step | 処理 | 件数 | ソース |
|---|---|---|---|
| 1 | ゲームタグ付与 | 40件 | Last.fm |
| 2 | Steam 説明文バックフィル | 30件 | Steam appdetails |
| 3 | YouTube VideoID 補完（`games.youtube_video_id`） | 20件 | YouTube Search |
| 4 | Steam 新規ゲーム追加 | 5件 | Steam Search |
| 5 | Steam OST appID 発見 | 50件 | Steam Search + appdetails |
| 6 | Steam OST ページ取得（トラック・作曲家） | 10件 | Steam ストアページ（HTML） |
| 7 | 報告済み動画確認（`games.youtube_flagged`） | — | — |
| 8 | Last.fm 作曲家類似度更新（未登録優先） | 30件 | Last.fm |

### 依存パッケージ更新（PR #58・#59）

- **GitHub Actions**: checkout/setup-node/setup-python/upload-artifact を v7 に統一
- **npm (web)**: next 16.2.10→16.2.11、react/react-dom 19.2.4→19.2.8、@types/node ^20→^26
